...
<div class="dashboard-header">
    ...
    <span class="fw-bold fs-4 d-flex align-items-center">
        ...
        <span class="fw-bold fs-4"><i class="fas fa-shopping-cart me-2"></i>Pedido #<?= $pedido['pedido_id'] ?></span>
    </div>
    <div class="user-info">
...
<div class="container-fluid">
    ...
    <div class="main-card card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            <span class="card-title">Pedido #<?= $pedido['pedido_id'] ?></span>
            <a href="<?= BASE_URL ?>admin/pedidos" class="btn btn-outline-secondary"><i class="fas fa-arrow-left"></i> Volver</a>
        </div>
        <div class="card-body">
            <div class="row mb-4">
                <div class="col-md-5 border-end">
                    ...
                    <ul class="list-unstyled mb-0">
                        <li><strong>Nombre:</strong> <?= htmlspecialchars($pedido['nombre_cliente']) ?></li>
                        <li><strong>Email:</strong> <?= htmlspecialchars($pedido['email_cliente']) ?></li>
                        <li><strong>Teléfono:</strong> <?= htmlspecialchars($pedido['telefono_cliente']) ?></li>
                        <li><strong>Dirección:</strong> <?= htmlspecialchars($pedido['direccion_cliente']) ?></li>
                    </ul>
                </div>
                <div class="col-md-7">
                    <h6 class="text-muted mb-3">Información del Pedido</h6>
                    <div class="mb-2"><strong>Fecha:</strong> <?= date('d/m/Y H:i', strtotime($pedido['fecha_pedido'])) ?></div>
                    <div class="mb-2"><strong>Estado:</strong> <span class="badge bg-<?= $pedido['estado'] == 'completado' ? 'success' : ($pedido['estado'] == 'pendiente' ? 'warning' : ($pedido['estado'] == 'procesando' ? 'info' : ($pedido['estado'] == 'enviado' ? 'primary' : 'secondary'))) ?>"><?= ucfirst($pedido['estado']) ?></span></div>
                    <div class="mb-2"><strong>Total:</strong> €<?= number_format($pedido['total'], 2) ?></div>
                </div>
            </div>
            <h6 class="text-muted mb-3 mt-4">Productos del Pedido</h6>
